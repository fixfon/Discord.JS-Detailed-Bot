...
module.exports = async (oldRole, newRole) => {
    ...
    if(!client.config.get(newRole.guild.id).get('logEvents') && !client.config.get(newRole.guild.id).get('logEvents').get('guildLog').enabled &&
    !client.config.get(newRole.guild.id).get('logEvents').get('guildLog').channelID) return

    const logCh = newRole.guild.channels.cache.find(ch => ch.id == client.config.get(newRole.guild.id).get('logEvents').get('guildLog').channelID)
    // const logCh = client.config.get(newRole.guild.id).get('logEvents').get('guildLog').channelID
    if (!logCh) return
